		...
		while True:
			...
			if not pq.empty():
				...
				if a[1] == 3:	#change accordingly
					...
					c.connect(addr)
					clk = clk +1
					data = str(msgtype)+"/"+str(amt)+"/"+str(dest)+"/"+str(3)+"/"+str(clk) #change accordingly
...
while True:
	...
	if x == 1:
		...
		if amt<= balance:
			...
			e = (("send", 0, amt, id, 3, clk)) # change accordingly
